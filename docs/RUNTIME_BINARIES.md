...
### الصيغ المقبولة (منذ v1.2 — مستخرج مرن)

يحاول المثبّت استيعاب أي طريقة عملية لتجميع الحزمة:

| بدلاً من الصيغة المرجعية | يُقبل أيضاً |
|---|---|
| ZIP (`.qbxruntime`) | **TAR** أو **TAR.GZ/TGZ** مباشرة (يُكشف النوع من البايتات الأولى) |
| الملفات بمستوى جذر الأرشيف | **غلاف مجلد واحد** حول المحتويات (أدوات الضغط الرسومية) |
| `imagefs.tar` | `rootfs.tar` أو `imagefs.tar.gz` / `imagefs.tgz` / `rootfs.tar.gz` |
| أسماء نظيفة | لاحقات إعادة التنزيل « `proot (1)` » تُتجاهل تلقائياً |
| tar جذرُه `./` | **جذر متداخل** `tar -cf imagefs.tar imagefs/` يُسطَّح تلقائياً بعد الفك |

> الشرط الثابت الوحيد: وجود **نظام جذر** (bin أو usr/bin داخله) + **proot** + **version.json**.
> عند الفشل تُعرض المكونات الناقصة بالاسم داخل رسالة الخطأ وفي سجل الجلسة.
...
| العرض | السبب المحتمل |
|---|---|
| «حزمة غير صالحة — ناقص: …» | تُعرض المكونات الناقصة بالاسم — تأكد من وجود نظام الجذر + proot + version.json داخل الأرشيف |
| **«لم يتم تثبيت وقت التشغيل بعد» رغم نجاح التثبيت** | نسخة تطبيق قديمة (targetSdk ≥ 29) — أندرويد 10+ يمنع تنفيذ ملفات التطبيق (W^X). **حدّث APK إلى 1.1.1+ ولا حاجة لإعادة تثبيت الحزمة** |
| «أندرويد يمنع تنفيذ مكوّنات المحاكي» عند التثبيت | اختبار التنفيذ الفعلي فشل — تأكد من تحديث التطبيق، وراجع السجل من الإعدادات |
